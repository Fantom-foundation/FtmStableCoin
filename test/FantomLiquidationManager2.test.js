...
contract('Unit Test for FantomLiquidationManager', function([
  ...
  describe('depositing collateral and minting fUSD', function() {
    ...
    it('Scenario 4', async function() {
      ...
      console.log(`
            Let's set the price of wFTM to 1.5 USD`);
      await this.mockPriceOracleProxy.setPrice(
        this.mockToken.address,
        etherToWei(1.5)
      );
      ...
      await expectRevert(
        this.fantomLiquidationManager.startLiquidation(borrower, {
          from: admin
        }),
    ...
    it('Scenario 5', async function() {
      ...
      console.log(`
            The amount of fUSD that bidder2 has spent is 10000 minus ${weiToEther(
              balance
            )}`);
      ...
      console.log(
        `
            The actual amount of wFTM that bidder1 receives: `,
        weiToEther(balance)
      );
      ...
      console.log(
        `
            The actual amount of wFTM that bidder2 receives: `,
        weiToEther(balance2)
      );
    ...
    it('Scenario 6', async function() {
      ...
      let liquidationDetails = await this.fantomLiquidationManager.getLiquidationDetails(
        1
      );
      console.log(
        `
      ...
      liquidationDetails = await this.fantomLiquidationManager.getLiquidationDetails(
        1
      );
      console.log(
        `
    ...
    it('Scenario 7', async function() {
      ...
      await this.mockToken.approve(this.fantomMint.address, etherToWei(5000), {
        from: borrower
      });
      await this.mockToken2.approve(this.fantomMint.address, etherToWei(4999), {
        from: borrower
      });
      ...
      console.log(`
            The amount of wFTM2 that borrower is refunded: ${weiToEther(
              balance
            )}`);
